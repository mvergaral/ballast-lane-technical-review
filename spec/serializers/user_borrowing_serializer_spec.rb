...
RSpec.describe UserBorrowingSerializer, type: :serializer do
  ...
  describe 'difference from BookBorrowingSerializer' do
    let(:book_borrowing_serializer) { BookBorrowingSerializer.new(borrowing) }
    let(:book_borrowing_json) { JSON.parse(ActiveModelSerializers::Adapter.create(book_borrowing_serializer).to_json) }
    let(:user_borrowing_json) { JSON.parse(serialization.to_json) }

    it 'includes book_id instead of user_id in attributes' do
      expect(user_borrowing_json['book_id']).to eq(borrowing.book_id)
      expect(user_borrowing_json['user_id']).to be_nil
      
      expect(book_borrowing_json['user_id']).to eq(borrowing.user_id)
      expect(book_borrowing_json['book_id']).to be_nil
    end

    it 'includes same associations structure' do
      expect(user_borrowing_json['user']).to eq(book_borrowing_json['user'])
      expect(user_borrowing_json['book']).to eq(book_borrowing_json['book'])
    end
  end
  ...
  describe 'collection serialization' do
    it 'can serialize multiple borrowings correctly' do
      # Create different books for each borrowing to avoid validation error
